...
class Camera:
    ...
    def draw_scores(self, scores, high_scores):
        pix_pos = 32, 32
        line_shift = 32

        for i, (score, high_score) in enumerate(zip(scores, high_scores)):
            score_surf = self._medium_font.render(f"SCORE P{i+1}: {score:,}", True, C_BLACK)
            score_pix_size = score_surf.get_size()
            self._screen.blit(score_surf, (
                    pix_pos[0],
                    self.res[1] - score_pix_size[1] - pix_pos[1] - (N_PLAYERS-i) * line_shift
                    ))

            high_score_surf = self._medium_font.render(f"HIGH SCORE P{i+1}: {high_score:,}", True, C_BLACK)
            high_score_pix_size = high_score_surf.get_size()
            self._screen.blit(high_score_surf, (
                    self.res[0] - high_score_pix_size[0] - pix_pos[0],
                    self.res[1] - high_score_pix_size[1] - pix_pos[1] - (N_PLAYERS-i) * line_shift,
                    ))
    ...
    def draw_end_screen(self, score, winner):
        ...
        self._screen.blit(score_text_surf, self.res / 2 - score_text_pix_size / 2)
